...
function closeAllModals() {
  const modals = ['contact-modal', 'edit-modal', 'tools-modal'];
  const overlay = document.getElementById('modal-overlay');
...
function sendEditSuggestion() {
  ...
  const subject = 'DomusMed - Sugestão de Edição de Conteúdo';
  const body = `Sugestão de edição: ${message}\n\nPágina: ${pageTitle}\nLink: ${pageUrl}`;
...
function setupModalListeners() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      const activeModal = document.querySelector('.contact-modal.active, .edit-modal.active, .tools-modal.active');
      if (activeModal) {
        closeAllModals();
      }
    }
